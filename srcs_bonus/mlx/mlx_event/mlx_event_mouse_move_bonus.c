...
int	mlx_event_mouse_move_bonus(int x, int y, void *param)
{
	...
	if (y != 0)
		set_cam_forward(&rt->cam->o_info, v3d_add(\
		rt->cam->o_info.tr.forward[AXIS_Z], \
		v3d_smult(rt->cam->o_info.tr.forward[AXIS_Y], \
		-y * MOUSE_SENSIVITY)));
	if (x != 0)
		set_cam_forward(&rt->cam->o_info, v3d_add(\
		rt->cam->o_info.tr.forward[AXIS_Z], \
		v3d_smult(rt->cam->o_info.tr.forward[AXIS_X], \
		-x * MOUSE_SENSIVITY)));
	rt->is_updated = 1;
	return (0);
}
