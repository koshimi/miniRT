...
int	get_min_point_idx(\
	t_ray ray_local, t_vector3_double *points, double *sqr_mag, int valid_bits);

//	returns valid bits
static int	get_circle_intersect_points(\
			t_ray ray, double r, double h, t_vector3_double *points)
{
	t_vector3_double	o;
	t_vector3_double	d;
	double				t1;
	...
	t2 = (-h / 2 - o.y) / d.y;
	valid_bits = 0;
	points[2] = v3d(o.x + t1 * d.x, h / 2, o.z + t1 * d.z);
	points[3] = v3d(o.x + t2 * d.x, -h / 2, o.z + t2 * d.z);
...
//	returns valid bits
static int	get_side_intersect_points(\
			t_vector3_double abc, t_ray ray, double h, t_vector3_double *points)
{
	double	disc;
	double	t1;
	double	t2;
	...
	points[0] = v3d(ray.origin.x + t1 * ray.dir.x, ray.origin.y \
		+ t1 * ray.dir.y, ray.origin.z + t1 * ray.dir.z);
	t2 = (-abc.y - disc) / (2 * abc.x);
	points[1] = v3d(ray.origin.x + t2 * ray.dir.x, ray.origin.y \
...
static int	has_side_intersect_points(\
			t_ray ray_local, double r, t_vector3_double *abc)
{
	t_vector3_double	d;
	t_vector3_double	o;
...
//	returns min_idx
static int	get_point_idx(\
			t_vector3_double *ret, t_ray ray_local, double r, double h)
{
		...
		t_ray ray_world, const t_o_cyl *cyl, unsigned int obj_idx)
{
	...
	if (2 <= hit_idx)
		hit.hit_normal = v3d_smult(\
			cyl->o_info.tr.forward[AXIS_Y], 1 - 2 * (hit_idx == 3));
	else
