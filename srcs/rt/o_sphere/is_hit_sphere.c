...
static int	get_closer_point(\
			t_vector3_double abc, t_ray ray, t_vector3_double *point)
{
	double				disc;
	double				t1;
	double				t2;
	...
	p1 = v3d(ray.origin.x + t1 * ray.dir.x, ray.origin.y \
		+ t1 * ray.dir.y, ray.origin.z + t1 * ray.dir.z);
	t2 = (-abc.y - disc) / (2 * abc.x);
	p2 = v3d(ray.origin.x + t2 * ray.dir.x, ray.origin.y \
