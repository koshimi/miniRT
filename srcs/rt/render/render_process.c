...
void	cast_ray_simple(t_rt_dat *rt)
{
	...
	unit_dist = get_ray_unit_dist(\
				rt->cam->o_info.tr.forward[AXIS_Z], WIN_X, rt->cam->fov);
	y = -1;
	while (++y < WIN_Y)
	{
		x = -1;
	...
	while (++y < WIN_Y)
	{
		...
		while (++x < WIN_X)
			if (x % DWNX != 0 || y % DWNY != 0)
				rt->col_buf[y][x] = rt->col_buf[y - y % DWNY][x - x % DWNX];
	}
	return ;
}
...
void	rt_draw_col_buf(t_rt_dat *rt)
{
	...
	while (++x < WIN_X)
	{
		y = -1;
		while (++y < WIN_Y)
			mlx_draw_pixel(rt->my_mlx, v2i(x, y), rt->col_buf[y][x]);
	}
}
